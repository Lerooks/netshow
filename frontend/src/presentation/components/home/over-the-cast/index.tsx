...
export const OverTheCastSection = observer(() => {
  const { overTheCastVideos, loadOverTheCastVideos } = useHomeStore();

  useEffect(() => {
    if (overTheCastVideos.length === 0) {
      loadOverTheCastVideos();
    }
  }, [overTheCastVideos.length, loadOverTheCastVideos]);
  ...
  return (
    ...
    >
      ...
      <Carousel videos={overTheCastVideos}></Carousel>
    </Box>
  );
});
